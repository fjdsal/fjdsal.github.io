    ...
    var swiper = new Swiper('#swiper-container', { // swiper ist das wenn die bilder groß werden und man wischen kann, hier verschiedene optionen
      ...
      pagination: { // die Punkte unter dem bild
    ...
    if (window.innerWidth <= 1100) { // damit man am handy nicht zoomen kann
    ...
    function closeGallery() {
      ...
      function onTransitionEnd() {   // es wird wieder alles css zurückgesetzt
    ...
    document.addEventListener('keydown', function (event) { //escape zum schließen möglich
      if (event.key === 'Escape') {
        closeGallery();
      }
    });
